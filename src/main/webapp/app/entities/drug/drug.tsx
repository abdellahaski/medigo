...
export const Drug = (props: IDrugProps) => {
  ...
  return (
    <div>
      <h2 id="drug-heading">
        <Translate contentKey="mediGoApp.drug.home.title">Drugs</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="mediGoApp.drug.home.createLabel">Create new Drug</Translate>
        </Link>
      </h2>
        ...
        {drugList && drugList.length > 0 ? (
            ...
            <tbody>
              {drugList.map((drug, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{drug.label}</td>
                  <td>{drug.type}</td>
                  <td>{drug.validated ? 'true' : 'false'}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="mediGoApp.drug.home.notFound">No Drugs found</Translate>
            </div>
          )
        )}
      </div>
