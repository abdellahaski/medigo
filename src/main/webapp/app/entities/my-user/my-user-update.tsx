...
export const MyUserUpdate = (props: IMyUserUpdateProps) => {
  const [doctorCabinetId, setDoctorCabinetId] = useState('0');
  const [assistantCabinetId, setAssistantCabinetId] = useState('0');
  const [isNew, setIsNew] = useState(!props.match.params || !props.match.params.id);

  const { myUserEntity, cabinets, loading, updating } = props;
        ...
        <Col md="8">
          {loading ? (
            ...
            <AvForm model={isNew ? {} : myUserEntity} onSubmit={saveEntity}>
              ...
              <AvGroup>
                <Label id="typeLabel" for="my-user-type">
                  <Translate contentKey="mediGoApp.myUser.type">Type</Translate>
                </Label>
                <AvField id="my-user-type" type="text" name="type" />
              </AvGroup>
              <AvGroup>
                <Label id="currentInsuranceLabel" for="my-user-currentInsurance">
                  <Translate contentKey="mediGoApp.myUser.currentInsurance">Current Insurance</Translate>
                </Label>
                <AvField id="my-user-currentInsurance" type="text" name="currentInsurance" />
              </AvGroup>
              <AvGroup>
                <Label for="my-user-doctorCabinet">
                  <Translate contentKey="mediGoApp.myUser.doctorCabinet">Doctor Cabinet</Translate>
                </Label>
                <AvInput id="my-user-doctorCabinet" type="select" className="form-control" name="doctorCabinet.id">
                  <option value="" key="0" />
                  {cabinets
              ...
              <AvGroup>
                <Label for="my-user-assistantCabinet">
                  <Translate contentKey="mediGoApp.myUser.assistantCabinet">Assistant Cabinet</Translate>
                </Label>
                <AvInput id="my-user-assistantCabinet" type="select" className="form-control" name="assistantCabinet.id">
                  <option value="" key="0" />
                  {cabinets
                    ? cabinets.map(otherEntity => (
                        <option value={otherEntity.id} key={otherEntity.id}>
